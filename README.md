...
An exact, deterministic Tetris rule core, Leela-style search, and a
self-play policy/value learning stack with TetraFormer as the reference model
and CNN/CNN+Transformer architectures available for controlled ablation.

Start with [`docs/README.md`](docs/README.md) for the documentation map.
`docs/SPEC.md` is intentionally preserved as the original design baseline;
later design changes are recorded in [`docs/adr/`](docs/adr/README.md) rather
than rewritten into the specification.
...
**Status:** the rule core, move generation/tokenization, search, self-play,
GPU training/inference, and Candidate/Champion Arena loop are implemented. The
active work is sample efficiency, architecture ablation, reproducible self-play
curricula, and later timing/MoE/interpretability work. See
...
```sh
make test         # dependency-free test suite
...
The normal `-Werror` test suite covers the simulator, search, dataset contract,
and C++/PyTorch parity. Highlights (spec §18):
...
Without them the parity fixture tests skip while the rest of the suite still runs.
...
tests/              dependency-free engine/search/data harness
...
trainer/
  ...
  gpu_match.py      GPU inference match runner with combat/speed diagnostics
...
docs/               documentation map, architecture, training protocol, ADRs
```
